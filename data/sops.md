...
### Resolution Steps
1. The FIRST action for congestion is ALWAYS: call `reroute_traffic(source_router="<affected_router>", target_router="Core-Router-Bangalore")`.
...
|--------|--------------|--------------|------|
| Core-Router-Mumbai | Primary-Link-A (Mumbai→Delhi) | Backup-Link-B (via Bangalore) | Core |
| Core-Router-Delhi | Primary-Link-A (Delhi→Mumbai) | Backup-Link-B (via Chennai) | Core |
| Core-Router-Bangalore | Primary-Link-A (Bangalore→Mumbai) | Backup-Link-B (via Chennai) | Core |
| Core-Router-Chennai | Primary-Link-A (Chennai→Bangalore) | Backup-Link-B (via Bangalore) | Core |
| Edge-Router-North | Primary-Link-A (Edge→Core-Delhi) | Backup-Link-B (via Core-Mumbai) | Edge |
| Edge-Router-South | Primary-Link-A (Edge→Core-Chennai) | Backup-Link-B (via Core-Bangalore) | Edge |

**Core routers** handle backbone traffic between cities. **Edge routers** connect end users.
When rerouting from a core router, always target a different core router (e.g., Core-Router-Bangalore).
...
**Resolution:** `reroute_traffic("Core-Router-Mumbai", "Core-Router-Bangalore")` — network_config.json updated: current_route → Backup-Link-B.
...
## Past Incident: INC-2025-0019 — Bangalore Power Outage
