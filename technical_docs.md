...
All tools are native Python functions decorated with LangChain's `@tool`. They are bound to the LLM so it knows their schemas. Following the "Aspen Pattern," these tools do *not* rely on external APIs; they use `json.dump` to immediately mutate `network_config.json`.
